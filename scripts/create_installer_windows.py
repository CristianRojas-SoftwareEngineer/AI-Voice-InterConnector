#!/usr/bin/env python3
"""
Genera un instalador Inno Setup para ai-voice-interconnector Windows.

Usage:
    python scripts/create_installer_windows.py <onedir_path> [--output <output_dir>]

Example:
    python scripts/create_installer_windows.py dist/ai-voice-interconnector
"""
...
def generate_iss(source_dir: Path, output_dir: Path, version: str, info_after: Path,
    ...
    """
    exe_name = "ai-voice-interconnector.exe"
    ...
    lines = []
    lines.append("; Inno Setup script for ai-voice-interconnector")
    lines.append("; Generated by scripts/create_installer_windows.py")
    lines.append("")
    lines.append("[Setup]")
    # AppId: Inno Setup escapa '{' con '{{', así que esto produce el literal
    # {E8A1B2C3-...} y la clave de desinstalación {AppId}_is1 coincide con la
    # que busca 'setup --uninstall' en cli.py. Una llave final extra ('}}')
    # generaba la clave {E8A1B2C3-...}}_is1 y rompía esa desinstalación.
    lines.append("AppId={{E8A1B2C3-D4F5-6789-ABCD-EF0123456789}")
    lines.append("AppName=ai-voice-interconnector")
    lines.append("AppVersion=" + version)
    lines.append("AppPublisher=ai-voice-interconnector")
    ...
    lines.append("DefaultDirName={localappdata}\\Programs\\ai-voice-interconnector")
    lines.append("DefaultGroupName=ai-voice-interconnector")
    ...
    lines.append("OutputBaseFilename=ai-voice-interconnector-" + version + "-x86_64-setup")
    ...
    lines.append("")
    lines.append("[Icons]")
    icon_suffix = "; IconFilename: {app}\\ai-voice-interconnector.ico" if has_icon else ""
    lines.append("Name: {group}\\ai-voice-interconnector; Filename: {app}\\ai-voice-interconnector.exe" + icon_suffix)
    lines.append("Name: {group}\\Uninstall ai-voice-interconnector; Filename: {uninstallexe}")
    lines.append("Name: {autodesktop}\\ai-voice-interconnector; Filename: {app}\\ai-voice-interconnector.exe" + icon_suffix)
    lines.append("")
    ...
    lines.append('Filename: {cmd}; Parameters: "/k ""{app}\\ai-voice-interconnector.exe"" setup"; Description: "Descargar ahora el modelo de voz (setup)"; Flags: postinstall skipifsilent runasoriginaluser nowait')
    lines.append("")
    lines.append("[Code]")
...
def info_after_text() -> str:
    ...
    return (
        ...
        "===========================\r\n\r\n"
        "ai-voice-interconnector ya está instalado, pero los modelos de voz (es-mx-latam\r\n"
        "y en, ~6 GB en total) NO vienen incluidos en el instalador: se\r\n"
        ...
        "fallarán de inmediato y te recordarán ejecutar 'ai-voice-interconnector setup'.\r\n\r\n"
        "El instalador añadió la carpeta de instalación al PATH de tu usuario, por lo\r\n"
        "que podrás invocar 'ai-voice-interconnector' por nombre en una terminal nueva.\r\n\r\n"
        "Código fuente (GPLv3)\r\n"
        "======================\r\n\r\n"
        "AI Voice InterConnector se distribuye bajo la GNU General Public License v3 o\r\n"
        "posterior. El código fuente completo está disponible públicamente bajo\r\n"
        "GPLv3 en el repositorio:\r\n\r\n"
        "    https://github.com/CristianRojas-SoftwareEngineer/AI-Voice-InterConnector\r\n\r\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Create ai-voice-interconnector Windows installer via Inno Setup"
    )
    parser.add_argument(
        "onedir",
        type=Path,
    ...
    exe_in_source = source_dir / "ai-voice-interconnector.exe"
    if not exe_in_source.exists():
        candidates = list(source_dir.glob("*/ai-voice-interconnector.exe")) + list(source_dir.glob("ai-voice-interconnector.exe"))
        if candidates:
            exe_in_source = candidates[0]
        else:
            print(f"ERROR: Could not find ai-voice-interconnector.exe in {source_dir}", file=sys.stderr)
            sys.exit(1)
    ...
    try:
        ...
        installer_name = f"ai-voice-interconnector-{version}-x86_64-setup.exe"
        ...
        if installer_path.exists():
            ...
            candidates = list(output_dir.glob("ai-voice-interconnector-*-x86_64-setup.exe"))
            if candidates:
                p = candidates[0]
