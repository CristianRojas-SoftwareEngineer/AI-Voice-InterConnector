...
| `target-v2` (`test-windows`/`test-linux`/`test-macos`/`coverage` `test`/`test`/`test`/`cov` + `build-*` `full`, `os: windows/linux/macos`) | `v2` | idem `registry`, variant-específico (`test`/`cov`/`full`) + `os`. Los crates propios del workspace (path dependencies) recompilan siempre por fingerprinting por `mtime` (parcial en los tres SO; total solo en Windows, que pierde también dependencias externas). `build-*` restaura `target-v2-full` y ejecuta `cargo clean -p ai-voice-interconnector` para determinismo del bump, ver determinismo debajo. |
