...
| `target-v2` (**solo `test-linux`/`coverage`**, `os: linux`) | `v2` | idem `registry`, variant-específico (`test`/`cov`) + `os`. `build-*` **no usa `target/`** (`cargo_restore_registry` solo `registry`+`sccache`), ver determinismo debajo. |
