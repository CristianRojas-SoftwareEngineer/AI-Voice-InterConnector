...
| `target-v2` (**solo `test-linux`/`coverage`/`build-*`**, `os: linux`) | `v2` | idem `registry`, variant-específico (`test`/`cov`/`full`) + `os`. Ver **determinismo de releases** debajo: `build-*` sobre tag `v*` invalida `target/release` vía `cargo clean --release` antes del build. |
...
> **Determinismo de releases (binary stale):** el transform `perl` que genera `Cargo.lock.cachekey` (`.circleci/config.yml:104`) neutraliza `version = "X.Y.Z"` del crate raíz (`ai-voice-interconnector` → `0.0.0`) para que el bump de versión del release no invalide la key exacta. Esto hace que `target-v2-…` no cambie entre releases que solo bumpean la versión, por lo que `cargo_restore_caches` puede restaurar un `target/release` de un release previo. Sin `cargo clean --release` el `cargo build --release --features full` reutiliza objetos `sccache` y linkea un binary stale (p. ej. omite el campo `version` en `version --json` introducido en `67fc1bc`). Por eso cada job `build-*` corre `cargo clean --release` (`.circleci/config.yml:833`, `1062`, `1206`, `1376`) antes de compilar sobre tags `v*`: inválida `target/release` de forma determinista, preservando `registry`/`sccache`/`target/debug` y el speedup parcial. El smoke-test de cada `build-*` valida `version --json` (`name`/`version`/`schema_version`) fail-fast.
