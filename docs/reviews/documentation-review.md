...
**`docs/` (12):** GOAL, ROADMAP, DESIGN, CLI-CONTRACT, DAEMON-MODE, DISTRIBUTION, SELF-HOSTED-INSTALL, BUILD, RELEASING, PARITY, MANUAL-VALIDATION, CLAUDE-CODE-INTEGRATION.

> **Nota de seguimiento:** los **siete hallazgos ya están resueltos** (ver marcas ✅ abajo). El conteo de `docs/` refleja el estado final: 14 → 13 tras la fusión de #6, y 13 → 12 tras eliminar `ARCHITECTURE.md` en #1.
...
## 🔴 1. `ARCHITECTURE.md` es un subconjunto obsoleto de `DESIGN.md` — un doc debería absorber al otro ✅ RESUELTO
...
**Resolución:** la sección «El entry point `bin/tts-sidecar`» se movió a `DESIGN.md` (con entrada en su ToC) y `ARCHITECTURE.md` fue eliminado. Se redirigieron las referencias entrantes: se quitó el bullet de `README.md §Documentación` y se apuntó la referencia de `BUILD.md` a `DESIGN.md`. El CHANGELOG conserva su mención histórica.

## 🔴 2. El árbol de estructura del proyecto está triplicado y los tres divergen ✅ RESUELTO
...
**Resolución:** el árbol canónico quedó en `DESIGN.md`, completado con `exceptions.py`, `exit_codes.py`, `synthetic_speech.py` y `daemon/__init__.py` (comentarios derivados de leer cada módulo). La copia de `ARCHITECTURE.md` desapareció con el archivo (#1) y la de `GOAL.md` se reemplazó por una referencia a `DESIGN.md#estructura-del-proyecto`.

## 🟠 3. El «Plan técnico … EJECUTADO (v0.6.0)» infla `ROADMAP.md` con historia terminada ✅ RESUELTO
...
**Resolución:** `ROADMAP.md` pasó de 355 a 76 líneas. El plan ejecutado se colapsó a un cierre de 8 líneas que declara la brecha resuelta en v0.6.0, señala el criterio de aceptación pendiente por feedback E2E y remite al CHANGELOG. Las secciones «Estado actual», «Trabajo pendiente» y «Hacia el Goal inmediato» quedaron intactas.

## 🟠 4. `SELF-HOSTED-INSTALL.md` es un plan mayormente ejecutado que ahora duplica a `DISTRIBUTION.md` + `PARITY.md` ✅ RESUELTO
...
**Resolución (opción «referencia»):** `SELF-HOSTED-INSTALL.md` pasó de 389 a 275 líneas. Se eliminó el andamiaje de plan (Entregable/Docs/Tests/Cierre/Orden de implementación/Validación E2E) y se reescribió en presente como referencia de la mecánica de los instaladores. Los duplicados con `PARITY.md` (desinstalación, antivirus) se colapsaron a punteros; se conservó el contenido único (glosario, flujos de script, endurecimiento del build). `PARITY.md` recibió un puntero de vuelta para la mecánica interna.

## 🟡 5. La explicación de MOTW / SmartScreen / Gatekeeper está repetida ~6 veces ✅ RESUELTO
...
**Resolución:** `SECURITY.md §Artefactos sin firmar` es ahora la sede canónica (explicación completa: MOTW/`ZoneId=3` en Windows, `com.apple.quarantine` en macOS, descarga por CLI, launcher local de PyPI, firma diferida). Los otros 6 archivos (README, DISTRIBUTION, SELF-HOSTED-INSTALL, BUILD, PARITY, GOAL) reemplazaron el razonamiento por un puntero a esa sección, conservando sus instrucciones operativas propias (`Unblock-File`, `xattr`, checksum, runbook WDSI).
...
- **`docs/`:** archivos planos que mezclan cinco géneros (spec ideal, estado vivo, contrato normativo, proceso, planes). No es un problema de ubicación *incorrecta* —todos pertenecen a `docs/`— pero si crece, una subdivisión ligera (`docs/design/`, `docs/ops/`) ayudaría. No es urgente.
...
Ninguno de estos cambios toca código ni contratos; son reorganización documental. **Los siete hallazgos quedaron resueltos.**