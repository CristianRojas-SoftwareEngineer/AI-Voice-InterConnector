...
- **Fecha:** 2026-09-04 (origen) — Aplicado 2026-09-05
- **Estado:** Aplicado — 7 rutas públicas (podadas `GET /voices:203` y `POST /voices/precompute:233`; sin legado; `GET /health` con `ct2`/`stt` aditivas) — reemplaza a `models-preload.md`
...
**Router `crates/avi-daemon/src/lib.rs:1124-1131`** — 7 rutas (podadas `GET /voices` y `POST /voices/precompute`; §5 aplicado):

- `GET /health:197` (`health_handler` `warm: warming|warm|warm_failed`, con `ct2`/`stt` aditivas `warm` cuando residentes — `enrich_health_body`)
...
- `POST /dub:1130` (`dub_handler:817` pipeline `transcribe→ct2→synthesize:371` bajo `synthesis_lock:321`, `#[cfg(feature="native-stt")]` determinista)
...
## 5. Gaps, sobrantes y optimizaciones — Aplicado 2026-09-05

**Sobrantes podados (daemon = acelerador) — sin legado:**

- `GET /voices:203` — podada (sin consumidor CLI; `voice list` es `VoiceStore` local `src/main.rs:493`; sin ruta `GET /voices`).
- `POST /voices/precompute:233` — podada íntegra (duplicaba `POST /voices/clone:1129`; sin función interna `ensure_qvoice` conservada; migración `reference.wav→.qvoice` se hace vía `clone_voice:253` directo).
...
- `GET /health:197` reporta `ct2: warm`/`stt: warm` además de `tts: warm` cuando `ct2_engine:87`/`stt_engine:77` están residentes (`enrich_health_body` determinista, sin `allow`).

## 6. Diseño ideal y recomendaciones

**Mantener daemon como acelerador con `warm`**, no como backend completo:

1. **Conservar 6 `Ambos` + 15 `Local-only` + 2 `Daemon-only`** (`CONTRACT.md:235-249` tabla `Comando|Delegable|Endpoint|Razón local-only: sin modelo`, `DAEMON.md:52-64` 7 rutas podadas).
2. **Podadas `GET /voices:203` y `POST /voices/precompute:233` íntegras** (sin función interna conservada; sin 5 CRUD remoto — solo si roadmap remoto explícito).
...
- `crates/avi-daemon/src/lib.rs:69-87` `DaemonState`, `lib.rs:197,310,464,555,670,817,1101,1124` handlers/router (7 rutas, sin `203,233`), `lib.rs:1169` `warmup_tts`, `enrich_health_body` determinista
