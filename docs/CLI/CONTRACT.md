...
- `VoiceStore` (`crates/avi-store/src/lib.rs`) reconoce una voz clonada por `reference.qvoice` (o `speech-reference.wav` legado); `timbre-reference.wav` es legado. Las voces de fábrica son `default` (clonada, con `reference.qvoice` graft) y `ryan`/`vivian` (presets puros sin referencia); `voice remove` las protege a las tres (exit 2; `crates/avi-store/src/lib.rs:16` `FACTORY_VOICES`, `src/main.rs:828-843` `is_factory_name`).
