...
**El dominio del eje son los códigos de fallo.** Quedan fuera, y es deliberado: el `0`, que no es un fallo y por tanto no tiene remedio del que hablar; el `130`, que es convención de señales (`128 + SIGINT`) y es correcto por otra razón —ante Ctrl+C el CLI ejecuta limpieza acotada del árbol del pidfile (o del PID en memoria sin pidfile, D-02) (2 s) y sale igualmente con 130, sin colgarse—; y el `1` de `doctor`, que usa el entero como canal de **veredicto** y no de fallo, porque el trabajo de ese comando *es* diagnosticar.
...
1. **Evento `started` inmediato**: se emite tras superar las validaciones baratas (parámetros, nombres, formatos) y justo antes de iniciar el trabajo pesado o la toma de locks.
2. **Latidos periódicos (`heartbeat`)**: el daemon emite latidos cada ~500 ms (y eventos de `progress` según la etapa) mientras el trabajo en segundo plano continúa.
3. **Evento final (`result` o `error`)**: transporta el resultado exitoso en su forma contractual (`{name, speech, timbre, precomputed: true}` en clonado; `{status: "dubbed", text, translated, audio_b64, voice}` en dub) o el objeto de error con su `reason` correspondiente.
...
**Gate `sin flags → exit 2`:** `cleanup` sin ningún flag de categoría (`--voices`, `--synthetic-speech`, `--model`, `--all`) sale con `2` `usage_error` sin borrar (`src/main.rs:2104`). `--all` equivale a `--voices --synthetic-speech --model` (`src/main.rs:2111-2113`).
...
`--all` es unión de limpiezas de datos; **no toca binario ni PATH** — solo `uninstall` borra binario y PATH (`src/main.rs:2635`). Con la raíz separada del registro de voces, el arrastre de `--voices` es código explícito y no un efecto del `rmtree`.
...
- `VoiceStore` (`crates/avi-store/src/lib.rs`) reconoce una voz clonada por `reference.qvoice` (o `speech-reference.wav` legado); `timbre-reference.wav` es legado. Las voces de fábrica son `default` (clonada, con `reference.qvoice` graft) y `ryan`/`vivian` (presets puros sin referencia); `voice remove` las protege a las tres (exit 2; `crates/avi-store/src/lib.rs:16` `FACTORY_VOICES`, `avi_store::is_factory_name` definida en `crates/avi-store/src/lib.rs:22`, invocada en `src/main.rs:834`).
...
Exige **exactamente una** de `{--audio, --mic}` (grupo mutuamente excluyente `required=True`, espejo de `speech transcribe`); `--duration N` solo es válido con `--mic` (con `--audio` sale con **2**). Con `--mic`, aplica el mismo push-to-talk con techo `AVI_PUSH_TO_TALK_MAX_SECS` (default 300 s, exit 0 al vencer) y aviso mínimo por stderr al iniciar la captura descritos en `speech transcribe`, por reutilizar su misma etapa de transcripción. `--source-language` es **requerido** (`es-latam`/`en`); `--target-language` (default `es-latam`) elige el idioma/modelo de síntesis y dispara la traducción si difiere del hablado; `-v/--voice` y `--temperature` son los de `say`. `--daemon`/`--no-daemon` aplican a la transcripción y a la síntesis (los tres modos, §5); sin flags, ambas etapas usan el daemon solo si responde.
