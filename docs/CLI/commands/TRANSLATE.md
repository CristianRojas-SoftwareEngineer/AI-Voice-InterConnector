...
Implementación: `handle_translate` (`src/main.rs:603`), despacho al daemon vía
`translate_via_daemon` (`src/main.rs:3402`) y `route_to_daemon`
(`src/main.rs:3314`), motor CT2 local en `avi-translation`
...
| `--from` | `es`\|`en` | `es` | Opcional. Idioma origen, alfabeto estricto del parser (`value_parser = ["es", "en"]`, `src/main.rs:150`) |
| `--to` | `es`\|`en` | `en` | Opcional. Idioma destino, mismo alfabeto estricto (`src/main.rs:152`) |
...
`crates/avi-daemon/src/lib.rs:676`), y en la taxonomía del grupo `speech`,
que no es este comando.

Sin `--daemon`/`--no-daemon` el modo es `Auto` (`DaemonMode::Auto`,
`src/main.rs:119-127`): se hace un probe de `GET /health` con timeout de 500 ms
(`probe_health`, `src/main.rs:3292`, vía `daemon_activo` en
`src/main.rs:3306`) y, si responde, se delega al
...
`handle_translate` (`src/main.rs:603-748`):

**Carril 1 — parser (`clap`, antes del handler).** `--from`/`--to` declaran
`value_parser = ["es", "en"]` (`src/main.rs:150-153`): cualquier otro valor
...
texto vacío (trim)? ──sí──► Err InvalidInput "empty_text" (exit 2)
    │ no
    ▼
resolve_stt_language(from/to)          ← normaliza "es-latam" → "es" (vivo solo vía IPC); el resto pasa verbatim (src/main.rs:79)
    │
    ▼
...
(`src/main.rs:619-627`). Es el único caso donde no se exige el modelo CT2
provisionado.
...
ningún modelo (`src/main.rs:663-672`).

---

## Despacho al daemon

`translate_via_daemon` (`src/main.rs:3402`) hace `POST /translate` con
...
`translate_handler` (`crates/avi-daemon/src/lib.rs:685`) replica la misma
...
Error, stdout (vía el manejador genérico de `main`, `src/main.rs:559-570`):
...
(`crates/avi-daemon/src/lib.rs:676`).
...
|---|---|---|
| valor fuera de alfabeto (`--from`/`--to` ∉ `{es, en}`) | 2 (rechazo del parser, sin `reason` de envelope) | `clap` rechaza el valor antes del handler (`value_parser`, `src/main.rs:150-153`); incluye `es-latam` vía CLI |
