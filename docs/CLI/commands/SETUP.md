...
**Implementación:** `cmd_setup` (linea 1871) despacha los tres modos exclusivos antes de cualquier lógica de provisión. El flujo normal delega en `model_cache.py` para inspección de caché y en `huggingface_hub.snapshot_download` para descargas con revisión fijada. La descarga de modelos de traducción usa `snapshot_download` + conversión CT2 vía `ctranslate2.TransformersConverter`. El modelo de transcripción (`faster-whisper-small`) se descarga sin conversión. El manejo de errores clasifica las excepciones en familias (credenciales, red, permisos, disco) con códigos de salida accionables.

**Proceso de ejecución:** Impresión de banner → integración de PATH (Linux) → chequeos de entorno → resolución de modelos a descargar según `--language` → pre-chequeo de espacio en disco (6 GB/modelo) → descarga por `snapshot_download` con revisión fijada → descarga de Voice Encoder si `es-mx-latam` fue provisionado → descarga de modelos de traducción (si aplica) → descarga de modelo de transcripción (si `--with-stt`) → limpieza de `.incomplete` huérfanos → emisión JSON (si `--json`).
