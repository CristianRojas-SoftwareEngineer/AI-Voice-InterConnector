...
- **Windows**: `uninstall` borra el directorio `%LOCALAPPDATA%\Programs\ai-voice-interconnector`, quita esa entrada del PATH de usuario (HKCU `Environment`, con `WM_SETTINGCHANGE`) y ejecuta `cleanup` para modelos/caché. **Un comando** (`ai-voice-interconnector uninstall --force`). El procedimiento manual (borrado/registry) queda como fallback si el binario está en uso.
