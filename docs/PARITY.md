...
Con la migración a Rust se reintrodujo la desinstalación en un comando: `ai-voice-interconnector uninstall` (único con binario+PATH; `cleanup --all` es unión Modelo+voces+habla sin binario ni PATH — funciones de uninstall y cleanup del binario principal, referenciadas en la sección 11 del contrato) es multiplataforma y espeja `setup --uninstall` del canal Python. El flujo interno es datos primero (`cleanup --all` como unión borra datos), integración de PATH después y binario al final, con dispatch por SO y confirmación interactiva (`--force`/`--yes` la omite; `cleanup` aporta `--dry-run` y gate `sin flags→2`):
