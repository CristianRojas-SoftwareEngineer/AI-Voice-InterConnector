...
Con la migración a Rust se reintrodujo la desinstalación en un comando: `ai-voice-interconnector uninstall` (único con binario+PATH; `cleanup --all` es unión Modelo+voces+habla sin binario ni PATH — `src/main.rs:132/318`, `docs/CLI/CONTRACT.md §11`) es multiplataforma y espeja `setup --uninstall` del canal Python. El flujo interno es datos primero (`cleanup --all` como unión borra datos), integración de PATH después y binario al final, con dispatch por SO y confirmación interactiva (`--force`/`--yes` la omite; `cleanup` aporta `--dry-run` y gate `sin flags→2`):
...
| *desinstalación en un comando* | Desinstalación | Windows + Linux | ✅ Cerrada (v0.10.8, reconciliada F6: `--all` = unión sin binario/PATH) | `uninstall` (binario+PATH) + `cleanup --all` (unión datos) multiplataforma — `src/main.rs:132/318` |
