...
Con la migración a Rust se reintrodujo la desinstalación en un comando: `ai-voice-interconnector uninstall` (único con binario+PATH; `cleanup --all` es unión Modelo+voces+habla sin binario ni PATH — funciones de uninstall y cleanup del binario principal, referenciadas en la sección 11 del contrato) es multiplataforma y espeja `setup --uninstall` del canal Python. El flujo interno es datos primero (mismo alcance que `cleanup --all`: `data_dir()`, snapshots HF y cachés `xet`/`ct2`/`.locks`), integración de PATH después y binario al final, con dispatch por SO y confirmación interactiva (`--force`/`--yes` la omite; `cleanup` aporta `--dry-run` y gate `sin flags→2`):

- **Windows**: `uninstall` borra el directorio `%LOCALAPPDATA%\Programs\ai-voice-interconnector` vía helper desacoplado determinista (`Wait-Process` + `Remove-Item -LiteralPath`, `crates/avi-daemon/src/spawn.rs:spawn_uninstall_helper` con `Stdio::null` + `DETACHED_PROCESS`), quita esa entrada del PATH de usuario con comparación canónica (`avi-store::canonical_path_key`, `HKCU` + `WM_SETTINGCHANGE`) y borra datos, modelos y cachés. **Un comando** (`ai-voice-interconnector uninstall --force`) sin aviso `Bórralo manualmente`.
...
| *desinstalación en un comando* | Desinstalación | Windows + Linux | ✅ Cerrada (v0.10.8, reconciliada: `--all` = unión sin binario/PATH) | `uninstall` (binario+PATH) + `cleanup --all` (unión datos) multiplataforma — funciones del binario principal |
