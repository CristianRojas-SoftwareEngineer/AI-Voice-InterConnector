...
impl Config {
    ...
    pub fn build(&mut self) -> PathBuf {
        ...
        {
            // Patch: conservar los flags de optimizacion (/O2, /Od) que pone el
            // crate `cc` en release; solo se descartan los flags de debuginfo.
            let skip_arg = |arg: &OsStr| match arg.to_str() {
                Some(s) => s == "-g" || s == "/Zi" || s == "/ZI",
                None => false,
            };
            let mut set_compiler = |kind: &str, compiler: &cc::Tool, extra: &OsString| {
                ...
                if generator.is_none() && msvc {
                    ...
                    if !self.defined(&flag_var_alt) {
                        ...
                        // Patch: garantizar optimizacion y asserts desactivados
                        // en builds Release/RelWithDebInfo (MSVC).
                        if build_type_upcase == "RELEASE" || build_type_upcase == "RELWITHDEBINFO" {
                            flagsflag.push(" /O2 /Ob2 /DNDEBUG");
                        }
                        cmd.arg(flagsflag);
                    }
                }
