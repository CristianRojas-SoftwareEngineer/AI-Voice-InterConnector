...
- For releases (`bump`/`tag`/`publish`), the `release` skill (`.claude/skills/release/SKILL.md`, `cargo run -p xtask -- release X.Y.Z`) is the source of truth: it bumps the 6 version carriers and **promotes** the hand-curated `## [No publicado]` CHANGELOG section to `## [X.Y.Z]` (it does not generate notes from `git log`; curate `[No publicado]` continuously during development). See `docs/RELEASING.md`.
<!-- </version_control> -->
