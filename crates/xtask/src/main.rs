...
enum Commands {
    ...
    /// Construye ONNX Runtime enlazado estáticamente en /MT (target/ort-mt/onnxruntime.lib)
    Ort,
}
...
/// Construye ONNX Runtime enlazado estáticamente en /MT para el motor STT Parakeet.
///
/// Reproduce la receta validada en el spike: clon superficial del tag `v1.28.1`,
/// configuración CMake con /MT, build Release del target `onnxruntime`, build
/// explícito de `re2` y `model_package` (no incluidos en `ALL_BUILD`), y fusión
/// manual vía `lib.exe`/`merge.rsp` que produce `target/ort-mt/onnxruntime.lib`,
/// consumible por el crate `ort` a través de `ORT_LIB_PATH`.
///
/// Solo ejecutable en Windows MSVC (Visual Studio 17 2022 + MSBuild + lib.exe).
#[allow(dead_code)]
fn run_ort_build() -> Result<()> {
    // El build de ORT es intrínsecamente Windows/MSVC: el generador de CMake es
    // "Visual Studio 17 2022", y los pasos finales usan msbuild + lib.exe.
    if !cfg!(windows) {
        anyhow::bail!(
            "xtask ort solo está soportado en Windows MSVC (Visual Studio 17 2022 + MSBuild + lib.exe)"
        );
    }

    // Tag pineado de ONNX Runtime, coherente con ort =2.0.0-rc.13 (serie 1.28).
    let ort_tag = "v1.28.1";
    ...
    // 1. Clon superficial del tag v1.28.1 (shallow clone = fast).
    if !src_dir.exists() {
        run_command(
            "git",
            &[
    ...
    // 2. Configuración CMake: generador VS 2022 x64 + flags /MT.
    //    - CMAKE_POLICY_DEFAULT_CMP0091=NEW: activa la política de runtime library
    //      (respetada a partir de CMake 3.15).
    //    - CMAKE_MSVC_RUNTIME_LIBRARY=MultiThreaded: fuerza el CRT estático /MT,
    //      coherente con +crt-static del workspace Rust (.cargo/config.toml).
    //    - ABSL_MSVC_STATIC_RUNTIME=ON: alina Abseil al /MT.
    //    - onnxruntime_BUILD_UNIT_TESTS=OFF: omite tests (ahorra tiempo de build).
    run_command(
        "cmake",
        &[
            "-B",
    ...
    // 3. Build Release del target `onnxruntime` (produye ort-build/Release/onnxruntime.lib).
    run_command(
        "cmake",
        &[
            "--build",
    ...
    // 4. Build explícito de re2 y model_package: NO están en ALL_BUILD, por lo que
    //    `cmake --build --target onnxruntime` no los compila. Se invoca msbuild
    //    directamente sobre los .vcxproj dentro del árbol de CMake (ort-build).
    run_command(
        "msbuild",
        &[
            &format!("{}\\re2.vcxproj", build_dir.display()),
    ...
    run_command(
        "msbuild",
        &[
            &format!("{}\\model_package.vcxproj", build_dir.display()),
            "/p:Configuration=Release",
        ],
    )?;

    // 5. Fusión manual: lib.exe consume un response file (merge.rsp) que lista
    //    onnxruntime.lib + model_package.lib (+ re2.lib) y produce
    //    target/ort-mt/onnxruntime.lib — la .lib estática /MT que ort enlaza
    //    vía ORT_LIB_PATH=target/ort-mt.
    std::fs::create_dir_all(out_dir)?;

    let merge_rsp = "merge.rsp";
    let libs = [
        format!("{}\\Release\\onnxruntime.lib", build_dir.display()),
        format!("{}\\Release\\re2.lib", build_dir.display()),
        format!("{}\\Release\\model_package.lib", build_dir.display()),
    ];
    std::fs::write(merge_rsp, libs.join("\n"))?;

    run_command(
        "lib.exe",
        &[
            &format!("/OUT:{}\\onnxruntime.lib", out_dir.display()),
            &format!("@{}", merge_rsp),
        ],
    )?;

    // 6. Verificar el artefacto de salida.
    let expected = out_dir.join("onnxruntime.lib");
    if !expected.exists() {
        anyhow::bail!(
            "La fusión no produjo {}: verifica que los .lib intermedios existan",
            expected.display()
        );
    }

    println!();
    println!("ONNX Runtime /MT construido con éxito:");
    println!("  {}", expected.display());
    println!();
    println!("Consumirlo en CI/local con:");
    println!("  ORT_LIB_PATH=target/ort-mt");
    Ok(())
}

/// Ejecuta un comando externo, propagando su salida a stderr y verificando el código de salida.
#[allow(dead_code)]
fn run_command(cmd: &str, args: &[&str]) -> Result<()> {
    eprintln!("[xtask ort] $ {} {}", cmd, args.join(" "));
    let status = std::process::Command::new(cmd)
        .args(args)
        .status()
        .map_err(|e| anyhow!("no se pudo ejecutar {}: {}", cmd, e))?;
    if !status.success() {
        anyhow::bail!(
            "comando falló (exit {}): {} {}",
