...
enum Commands {
    ...
    /// Genera un Cargo.lock normalizado (versión del crate neutralizada) para la clave de caché
    CacheKey {
        #[arg(long, value_name = "FILE", default_value = "Cargo.lock.cachekey")]
        out: PathBuf,
    },
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::CacheKey { out } => {
            let lock_text = std::fs::read_to_string("Cargo.lock")
                .map_err(|e| anyhow!("no se pudo leer Cargo.lock: {}", e))?;
            let normalized = normalize_cache_key(&lock_text)?;
            std::fs::write(&out, normalized)
                .map_err(|e| anyhow!("no se pudo escribir {}: {}", out.display(), e))?;
            eprintln!("Clave de caché normalizada escrita: {}", out.display());
        }
...
/// Neutraliza la versión del propio crate en el contenido de un `Cargo.lock` para
/// derivar una clave de caché estable entre releases: el checksum del insumo debe
/// depender solo de las dependencias reales, no del bump de versión de cada corte.
/// Dos locks que difieren únicamente en `version = "X.Y.Z"` del crate
/// `ai-voice-interconnector` normalizan a salida idéntica; cualquier cambio real de
/// dependencias (altas, bajas o bumps de deps) produce salida distinta. El resto del
/// contenido queda intacto.
fn normalize_cache_key(lock_text: &str) -> Result<String> {
    let re = Regex::new(r#"(name\s*=\s*"ai-voice-interconnector"\s*\nversion\s*=\s*")([^"]+)(")"#)
        .unwrap();
    if !re.is_match(lock_text) {
        anyhow::bail!("no se encontró el paquete ai-voice-interconnector en Cargo.lock");
    }
    let out = re.replace(lock_text, |caps: &regex::Captures| {
        format!("{}0.0.0{}", &caps[1], &caps[3])
    });
    Ok(out.into_owned())
}
...
mod tests {
    ...
    #[test]
    fn test_cache_key_invariante_a_version_del_crate() {
        let lock_a = r#"# This file is automatically @generated by Cargo.
version = 4
...
version = "1.0.86"
"#;
        // Solo difiere en la versión del propio crate → debe normalizar idéntico.
        let lock_b = lock_a.replace(r#"version = "0.17.1""#, r#"version = "0.18.0""#);
        let na = normalize_cache_key(lock_a).unwrap();
        let nb = normalize_cache_key(&lock_b).unwrap();
        assert_eq!(na, nb, "el bump de versión del crate no debe afectar la clave");

        // Un cambio real de dependencia → salida divergente.
        let lock_c = lock_a.replace(
            "name = \"anyhow\"\nversion = \"1.0.86\"",
            "name = \"anyhow\"\nversion = \"1.0.99\"",
        );
        let nc = normalize_cache_key(&lock_c).unwrap();
        assert_ne!(na, nc, "un cambio de dependencia debe cambiar la clave");
    }
