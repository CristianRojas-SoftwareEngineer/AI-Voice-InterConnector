...
<!-- <overview> -->
Reusable task command for the full manual E2E on the **Windows dev machine** as a final user. Each invocation is interpreted as a request to run `install → setup → daemon → voice clone → synthesis matrix → store/transcribe/translate/dub → cleanup/uninstall` with zero-residue, validating the published `vX.Y.Z` without depending on CI. Evolved from `.claude/plans/2026-08-27-prueba-e2e-windows-completa.md` (0.15.2) to `0.18.5` with `exe_dir` resolution (`crates/avi-tts/src/lib.rs:151`) and detached launch without pipe inheritance (`crates/avi-daemon/src/spawn.rs:30`).
<!-- </overview> -->
...
- **Cost:** `~9 GB` base (`qwen3-tts-0.6b` + `parakeet-tdt-v3` + `marian-es-en/en-es`) and `~11.5 GB` with `--with-base` (`qwen3-tts-0.6b-base` `crates/avi-store/src/lib.rs:410`). Warn before downloading.
- **State:** repo stays unmutated (`docs/`, `Cargo`, `.circleci` untouched). System under test is the installed artifacts (`%LOCALAPPDATA%\Programs\ai-voice-interconnector`, `%APPDATA%\ai-voice-interconnector\data`, `%USERPROFILE%\.cache\huggingface\hub` `crates/avi-store/src/lib.rs:446`).
...
Execute in order. Each task needs the previous task's artifacts (binary → models → daemon → `.qvoice` → persisted wav). Do not parallelize. Report `PASS/FAIL` per step with command, exit code, and payload.

### 0. Resolve current release
...
1. **Proceso seguido** — tasks executed in order `T0→T7` with commands, exit codes, and `PASS/FAIL` per step.
...
No mutations to `docs/`, `Cargo`, `.circleci` — report only. If models already provisioned, skip `setup` download and note `already provisioned`.
...
- [ ] `uninstall --force` leaves zero residue (`where` fails, `data_dir` empty, `HKCU` entry removed)

## Constraints

- Run only on the Windows dev machine as the final user; do not mock `curl`/`sha256sum` or network (unlike `tests/installer`).
- The `~9 GB` download is expected cost — not an incident. Use `hound` for `WAV` validation and `ParakeetEngine` for `WER` when compiled with `native-stt`.
